...
class GAEncoder(nn.Module):
    def __init__(self, ipa_conf):
        ...
        # factored token posterior embedding: 6 dims × 4 levels = 24-dim input
        self.token_posterior_embedder = nn.Linear(24, self._ipa_conf.c_s)

        # output heads
        self.token_code_net = nn.Sequential(
            nn.Linear(self._ipa_conf.c_s, self._ipa_conf.c_s), nn.ReLU(),
            nn.Linear(self._ipa_conf.c_s, self._ipa_conf.c_s), nn.ReLU(),
            nn.Linear(self._ipa_conf.c_s, 6 * 4),
        )
    ...
    def forward(self, t, rotmats_t, trans_t, codes_t, node_embed, edge_embed,
                generate_mask, res_mask, return_node_embed=False):
        """
        codes_t: (B, L, 6, 4) factored token posterior probabilities
        """
        num_batch, num_res = codes_t.shape[:2]
        codes_flat = codes_t.reshape(num_batch, num_res, 24)
        codes_flat = codes_flat * 2 - 1  # rescale [0,1] → [-1,1]
        ...
        node_embed = self.res_feat_mixer(torch.cat([
            node_embed,
            self.token_posterior_embedder(codes_flat),
        ...
        pred_codes_logits = self.token_code_net(node_embed).reshape(num_batch, num_res, 6, 4)
        pred_aa_logits = self.aa_head(node_embed)

        if return_node_embed:
            return pred_rotmats1, pred_trans1, pred_codes_logits, pred_aa_logits, node_embed
        return pred_rotmats1, pred_trans1, pred_codes_logits, pred_aa_logits
