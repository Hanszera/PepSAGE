...
from core.models.token_decoder import TokenStructureDecoder

K_TOKEN = 4   # FSQ levels per dimension
D_TOKEN = 6   # number of FSQ dimensions
...
class BFNModel(BFNBase):
    def __init__(self, cfg):
        ...
        # token structure decoder (dual-path)
        ...
        self.beta1_token = getattr(cfg, 'beta1_token', 1.2)
    ...
    def forward(self, batch, t=None):
        ...
        # extract factored token targets
        with torch.no_grad():
            token_targets = self.tokenizer_bridge.prepare_factored_targets(batch)
        ...
        factored_targets = token_targets["factored_targets"]  # (B, L, 6)
        target_valid = token_targets["valid_mask"]  # (B, L)
        gt_local_atoms = token_targets["gt_local_atoms"]  # (B, L, 15, 3)

        targets_onehot = F.one_hot(
            factored_targets.clamp(min=0), K_TOKEN
        ).float()  # (B, L, 6, 4)
        ...
        with torch.no_grad():
            ...
            # factored token BFN update
            if self.sample_tokens:
                codes_theta = self.factored_discrete_update(
                    t, beta1=self.beta1_token, x=targets_onehot, K=K_TOKEN
                )  # (B, L, 6, 4)
                codes_t = torch.where(
                    batch['generate_mask'][..., None, None].bool(),
                    codes_theta, targets_onehot
                )
            else:
                codes_t = targets_onehot.detach().clone()

        # denoise
        pred_rotmats_1, pred_trans_1, pred_codes_logits, pred_aa_logits, final_node_embed = \
            self.ga_encoder(
                t, rotmats_t, trans_t_c, codes_t,
                node_embed, edge_embed, gen_mask, res_mask,
                return_node_embed=True,
            )
        pred_codes_prob = F.softmax(pred_codes_logits, dim=-1)
        ...
        # factored token BFN loss
        token_mask = gen_mask * target_valid.long()
        if self.sample_tokens and token_mask.sum() > 0:
            token_bfn_loss = self.factored_discrete_loss(
                i=i, N=self.discrete_steps, beta1=self.beta1_token,
                one_hot_x=targets_onehot, p_0=pred_codes_prob,
                K=K_TOKEN, n_samples=200, mask=token_mask,
            )
        else:
            token_bfn_loss = torch.tensor(0.0, device=device)
        ...
        # structure decode loss
        pred_codes_hard = pred_codes_prob.argmax(dim=-1)  # (B, L, 6)
        atom_mask = batch['mask_heavyatom'].bool() & gen_mask.bool().unsqueeze(-1)
        if atom_mask.any():
            _, pred_local = self.token_decoder.decode_primary(
                pred_codes_hard, final_node_embed, pred_rotmats_1, pred_trans_1,
            )
    ...
    def sample(self, batch, num_steps, pos_norm):
        ...
        # token prior: uniform 1/K per dimension
        codes_0 = torch.ones((num_batch, num_res, D_TOKEN, K_TOKEN), device=device) / K_TOKEN
        # context residues get their GT codes
        token_targets = self.tokenizer_bridge.prepare_factored_targets(batch)
        if token_targets is not None:
            ctx_mask = ~gen_mask.bool() & token_targets["valid_mask"]
            ctx_onehot = F.one_hot(
                token_targets["factored_targets"].clamp(0), K_TOKEN
            ).float()
            codes_0[ctx_mask] = ctx_onehot[ctx_mask]

        rotmats_t, trans_t, codes_t = rotmats_0, trans_0, codes_0
        clean_traj = []
        ...
        for step_i in trange(1, num_steps + 1):
            ...
            pred_R1, pred_t1, pred_codes_logits, pred_aa_logits = self.ga_encoder(
                t, rotmats_t, trans_t, codes_t,
            ...
            # factored token update
            if self.sample_tokens:
                codes_t = self.factored_discrete_update(
                    t_next, beta1=self.beta1_token, x=pred_codes_prob, K=K_TOKEN
                )
                codes_t = torch.where(
                    gen_mask[..., None, None].bool(), codes_t, codes_0
                )
            else:
                codes_t = codes_0.detach().clone()

            clean_traj.append({
                "trans": pred_t1.detach().clone().cpu() * pos_norm,
                "rotmats": pred_R1.detach().clone().cpu(),
                "codes_prob": pred_codes_prob.detach().clone().cpu(),
        ...
        pred_R1, pred_t1, pred_codes_logits, pred_aa_logits, final_node_embed = \
            self.ga_encoder(
                t, rotmats_t, trans_t, codes_t,
                node_embed, edge_embed, gen_mask.long(), res_mask.long(),
                return_node_embed=True,
        ...
        final_codes = pred_codes_logits.argmax(dim=-1)  # (B, L, 6)
        pred_aa = F.softmax(pred_aa_logits, dim=-1)

        # decode to atom coordinates
        pos_heavyatom, pred_local = self.token_decoder.decode_primary(
            final_codes, final_node_embed,
        ...
        final_sample = {
            ...
            "codes": final_codes.detach().clone().cpu(),
    ...
    def fix_seq_sample(self, batch, num_steps, pos_norm):
        ...
        final_codes = pred_codes_logits.argmax(dim=-1)
        ...
        token_errors = {}
        if token_targets is not None:
            gt_codes = token_targets["factored_targets"]  # (B, L, 6)
            valid = gen_mask.bool() & token_targets["valid_mask"]
            if valid.any():
                for d in range(D_TOKEN):
                    correct = (final_codes[:, :, d] == gt_codes[:, :, d]) & valid
                    token_errors[f'val/token_acc_dim{d}'] = correct.sum().item() / valid.sum().item()
                all_correct = ((final_codes == gt_codes).all(dim=-1)) & valid
                token_errors['val/token_acc_joint'] = all_correct.sum().item() / valid.sum().item()

        # structure decode error
        pos_heavyatom, pred_local = self.token_decoder.decode_primary(
            final_codes, final_node_embed, rotmats_final, trans_final,
        )
        atom_mask = batch['mask_heavyatom'].bool() & gen_mask.bool().unsqueeze(-1)
        if atom_mask.any():
