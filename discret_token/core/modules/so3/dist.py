from core.modules.so3.utils import *
import logging
import os
...
def sample_matrix_fisher_torch(lambda_val, device='cpu'):
    ...
    if small_lambda > 0:
        ...
        while total < N_s:
            ...
            density = torch.exp(lambda_val_s * tr)
            u = torch.rand(batch, device=device)
            accept = R[u < density / max_density]
            if accept.numel() > 0:
                samples.append(accept)
                total += accept.shape[0]
        R_samples_small = torch.cat(samples, dim=0)[:N_s].reshape(-1,T, 3,3)
    else:
        R_samples_small=None
    N_b = N -N_s
    if N_b>0:
        # 李代数高斯近似
        sigma = 1.0 / math.sqrt(lambda_val)  # 经验近似
        omega = torch.randn(N_b, 3, device=device) * sigma
        R_samples_big = exp_so3(omega)
    else:
        R_samples_big = None
    # WARN: 
