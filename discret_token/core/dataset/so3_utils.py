...
def SO3_gaussian(gamma, rotmats_1, rotmats_base=None):
    """
    gamma: scalar ∈ [0, 1]
    rotmats_1: [B, 3, 3] target rotations
    ...
    rotvec_1 = calc_rot_vf(rotmats_base, rotmats_1)  # shape [B, 3]
    # Gaussian noise in so(3)
    noise = torch.randn_like(rotvec_1)  # shape [B, 3]
    # Weighted interpolation in tangent space
    rotvec_t = gamma * rotvec_1 + torch.sqrt(gamma * (1 - gamma)) * noise
    # Exp back to SO(3)
    rotmats_t = torch.matmul(rotmats_base, rotvec_to_rotmat(rotvec_t))
    return rotmats_t
