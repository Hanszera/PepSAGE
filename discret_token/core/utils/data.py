...
from core.modules.protein.constants import PAD_RESIDUE_INDEX
...
def repeat_batch(batch: Any, times: int) -> Any:
    if torch.is_tensor(batch):
        # batch.shape == [1, ...], 复制第 0 维 times 次
        repeats = [times] + [1] * (batch.dim() - 1)
        return batch.repeat(*repeats)
    elif isinstance(batch, dict):
        return {k: repeat_batch(v, times) for k, v in batch.items()}
    elif isinstance(batch, list):
        if len(batch) > 0:
            ele = batch[0]
            if isinstance(ele, tuple):
                # 如果是元组，复制每个元素
                return [item*times for item in batch]
            return batch*times
    else:
        # 其它类型原样返回
        return batch