...
def parse_config(
    ...
    def constructor_subs_variables(loader, node):
        ...
        if match:
            ...
            for g in match:
                vname, sep, default_val = g.partition(":-")
                has_cli_value = vname in subs_dict and len(str(subs_dict[vname])) > 0
                has_default = len(sep) > 0
                if has_cli_value:
                    resolved_value = subs_dict[vname]
                elif has_default:
    ...
    def constructor_envsubs_variables(loader, node):
        ...
        if match:
            ...
            for g in match:
                vname, sep, default_val = g.partition(":-")
                has_cli_value = vname in subs_dict and len(str(subs_dict[vname])) > 0
                env_value = os.environ.get(vname)
                has_env_value = env_value is not None and len(str(env_value)) > 0
                has_default = len(sep) > 0
                if has_cli_value:
                    resolved_value = subs_dict[vname]
                elif has_env_value:
