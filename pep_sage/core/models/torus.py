...
def tor_random_normal(t, dtype=None, device=None) -> torch.Tensor:
    z = torch.randn_like(t, dtype=dtype, device=device)
    return z
...
def tor_geodesic_gaussian(gamma, angles_1, base=None):
    '''Compute the geodesic on the torus with a Gaussian perturbation.'''
    if  base is None:
        base = torch.zeros_like(angles_1)
    tangent_vec = gamma * tor_logmap(base, angles_1)
    tangent_vec += tor_random_normal(angles_1) * torch.sqrt(gamma * (1 - gamma))
    return tor_expmap(base, tangent_vec)
