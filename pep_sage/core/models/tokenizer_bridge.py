...
from exp1_local_frame_tokenizer.exp1_tokenizer.model import LocalFrameTokenizer
from exp1_local_frame_tokenizer.exp1_tokenizer.pepsage_compat import (
    BBHeavyAtom,
    construct_3d_basis,
    global_to_local,
)
...
class TokenizerBridge(nn.Module):
    ...
    @torch.no_grad()
    def prepare_continuous_targets(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor] | None:
        """Extract continuous R^6 pre-quantization targets from ground truth structures.

        Returns:
            z_targets:      (B, L, 6) float, pre-quantization continuous R^6 values
            mean_codes:     (B, L, 128) projected code vectors (from quantized z)
            valid_mask:     (B, L) bool
        ...
        z_targets = torch.zeros((batch_size, num_res, D), device=device)
        ...
        gt_local_atoms = global_to_local(gt_frames, gt_frame_trans, batch["pos_heavyatom"])
