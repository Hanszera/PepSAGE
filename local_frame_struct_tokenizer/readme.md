...
```powershell
cd PepSAGE\local_frame_struct_tokenizer
```
...
The data paths in the default configuration are resolved relative to the repository root, `PepSAGE`.

Default Peptide/PepSAGE data locations:

```text
PepSAGE\data\pepsage\crude
PepSAGE\data\pepsage\processed
```

Default general protein data location:

```text
PepSAGE\data\general_protein
```
